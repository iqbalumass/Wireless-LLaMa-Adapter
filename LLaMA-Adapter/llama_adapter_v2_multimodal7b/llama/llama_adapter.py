...
import clip
from models.radar_fusion_encoder import RadarEncoder
import torch
...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        # --- Radar integration ---
        self.radar_encoder = RadarEncoder(embed_dim=512)
        self.radar_proj = nn.Linear(512, v_embed_dim)  # 512→768 to match CLIP embed dim
        
        try:
            state = torch.load("/work/pi_mshao_umassd_edu/Iqbal/RadarCLIP/project/checkpoints/radar_encoder_contrastive_512_safe.pth", map_location="cpu")
            self.radar_encoder.load_state_dict(state["radar_encoder"], strict=False)
            print("[Radar] Loaded pretrained radar encoder weights.")
        except Exception as e:
            print(f"[Radar] Warning: could not load radar weights: {e}")
        
        # Freeze radar encoder for now
    ...
    def forward_visual(self, imgs, radar=None):
        clip_feats = self.clip_encode_image(imgs)
        clip_feats = self.clip_proj_norm(self.clip_proj(clip_feats.float()))

        # --- fuse radar if provided ---
        if radar is not None:
            radar_embed = self.radar_encoder(radar)           # [B,512]
            radar_embed = self.radar_proj(radar_embed)        # [B,768]
            radar_tokens = radar_embed.unsqueeze(1).repeat(1, clip_feats.size(1), 1)
            clip_feats = torch.cat([clip_feats, radar_tokens], dim=1)  # [B, N_img+N_img, 768]
    ...
    def forward(self, tokens, labels, imgs, radar=None):
        visual_query = self.forward_visual(imgs, radar=radar)

        _bsz, seqlen = tokens.shape

        tokens = tokens.to(self.llama.tok_embeddings.weight.device)
