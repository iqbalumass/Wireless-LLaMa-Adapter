...
def init_distributed_mode(args):
    """Robust distributed initialization with single-GPU fallback."""

    # 🔹 Case 1: no distributed environment variables — run single GPU
    distributed_env_vars = ["RANK", "WORLD_SIZE", "SLURM_PROCID", "OMPI_COMM_WORLD_RANK"]
    if not any(k in os.environ for k in distributed_env_vars):
        print("⚙️  No distributed environment detected — using single-GPU mode.")
        args.rank = 0
        args.gpu = 0
        args.world_size = 1
        ...
        return  # 🔸 stop here — DO NOT call init_process_group

    # 🔹 Case 2: distributed environment detected
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        print("⚙️  Unknown distributed environment, running single GPU.")
        args.rank = 0
        args.gpu = 0
        args.world_size = 1
        args.distributed = False
    ...
    print(f"| distributed init (rank {args.rank}) on GPU {args.gpu}")
